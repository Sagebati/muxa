...
**Using muxa as a library inside a larger app:** don't ship a second, dedicated `muxa.toml` — fold muxa's plugin config into the host app's own config file under one nested table instead. Build one figment yourself (own file, own env prefix), extract your own app config from it, then hand it to `App::with_figment_at(figment, "muxa")` (or the file-path sugar `App::with_config_file_and_root(path, "muxa")`) — plugins then read `[muxa.web]`, `[muxa.otel]`, etc. instead of top-level `[web]`, `[otel]`, avoiding both the extra file and name collisions with the host's own top-level keys. `nested_figment` (`muxa-core/src/config.rs`) is the underlying rescoping primitive, built on `figment::Figment::find_value` + `Serialized::defaults`; only the `root` subtree becomes visible to plugins afterward, so extract your own struct from the original figment *before* scoping. The `env`/`RunMode` key is deliberately still resolved from the pre-scope figment, so it keeps working wherever the host naturally puts it (their own top level), regardless of nesting. See `crates/muxa/examples/hello.rs` + `examples/app.toml` for the end-to-end pattern.
...
- `muxa` — facade; its `examples/` (cargo-native, not a workspace member) hold the end-to-end demos (`hello` = otel+sqlite+web; `web_only` = minimal; `diesel_widgets` = diesel/Postgres JSON API with embedded migrations, emitting OTLP traces/metrics/logs to a `grafana/otel-lgtm` collector — both via docker-compose).
